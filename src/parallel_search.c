...
str_t pidSearchDelegate(char* pid_str, const int size, size_t pid_num) {
  ...
  node_search_t* node_num =
      mmap(NULL, pid_num * sizeof(node_search_t), PROT_READ | PROT_WRITE,
...
size_t getNumPid(const int size, const int step) {
  ...
  if (pid_num > max_pids.rlim_max) {
    pid_num = max_pids.rlim_max;
  }
...
str_t search(char* str, const int size) {
  ...
  char* pid_str = mmap(NULL, size, PROT_READ | PROT_WRITE,
                       MAP_SHARED | MAP_ANONYMOUS, -1, 0);
